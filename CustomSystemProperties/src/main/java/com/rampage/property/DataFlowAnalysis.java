...
public class DataFlowAnalysis {
    ...
    public static boolean judgeReflectionInMethod(SootMethod targetMethod, Map<String, Object> detail) {
        ...
        // judge return target reflection
        for (ReturnStmt returnStmt: returnStmts) {
            Value returnValue = returnStmt.getOp();
            if (returnValue.getType().toString().equals("java.lang.Class") && returnValue instanceof Local) {
                List<Unit> defUnits = findDefs(body, returnStmt, (Local) returnValue);
                for (Unit defUnit: defUnits) {
                    if (defUnit instanceof AssignStmt && ((AssignStmt) defUnit).getRightOp() instanceof InvokeExpr) {
                        SootMethod invokeMethod = ((InvokeExpr) ((AssignStmt) defUnit).getRightOp()).getMethod();
                        if (invokeMethod.getSignature().equals("<java.lang.Class: java.lang.Class forName(java.lang.String)>")) {
                            Value firstArg = ((InvokeExpr) ((AssignStmt) defUnit).getRightOp()).getArg(0);
                            // TODO: add inter procedure of reflectedClass
                            List<String> reflectedClassNames = retrieveStringValue(body, (Stmt) defUnit, firstArg, new ArrayList<>(), false, detail);
            ...
            else if (returnValue.getType().toString().equals("java.lang.reflect.Method")) {
                List<Unit> methodDefs = findDefs(body, returnStmt, (Local) returnValue);
                for (Unit methodDef: methodDefs) {
                    if (methodDef instanceof AssignStmt && ((AssignStmt) methodDef).getRightOp() instanceof InvokeExpr) {
                        SootMethod methodDefMethod = ((InvokeExpr) ((AssignStmt) methodDef).getRightOp()).getMethod();
                        if (methodDefMethod.getSignature().equals("<java.lang.Class: java.lang.reflect.Method getMethod(java.lang.String,java.lang.Class[])>")) {
                            Value methodValue = ((InvokeExpr) ((AssignStmt) methodDef).getRightOp()).getArg(0);
                            List<String> reflectedMethodNames = retrieveStringValue(body, (Stmt) methodDef, methodValue, new ArrayList<>(), false, null);
                            Value classValue = ((InvokeExpr) ((AssignStmt) methodDef).getRightOp()).getUseBoxes().get(0).getValue();
                            List<Unit> classDefs = findDefs(body, (Stmt) methodDef, (Local) classValue);
                            for (Unit classDef: classDefs) {
                                if (classDef instanceof AssignStmt && ((AssignStmt) classDef).getRightOp() instanceof InvokeExpr) {
                                    SootMethod classDefMethod = ((InvokeExpr) ((AssignStmt) classDef).getRightOp()).getMethod();
                                    if (classDefMethod.getSignature().equals("<java.lang.Class: java.lang.Class forName(java.lang.String)>")) {
                                        Value firstArg = ((InvokeExpr) ((AssignStmt) classDef).getRightOp()).getArg(0);
                                        // TODO: add inter procedure of reflectedClass
                                        List<String> reflectedClassNames = retrieveStringValue(body, (Stmt) classDef, firstArg, new ArrayList<>(), false, null);
                                        if (!reflectedClassNames.isEmpty() && AppConfig.acceptedClasses.stream().anyMatch(reflectedClassNames::contains)) {
                                            return true;
                                        }
                                    }
    ...
    public static boolean judgeReflectionInField(SootField targetField, Map<String, Object> detail) {
        boolean result = false;
        ...
        if (targetMethod != null) {
            ...
            for (Unit unit : body.getUnits()) {
                Stmt stmt = (Stmt) unit;
                if (stmt instanceof AssignStmt && ((AssignStmt) stmt).getLeftOp() instanceof FieldRef) {
                    ...
                    if (field.getSignature().equals(targetField.getSignature())) {
                        ...
                        if (field.getType().toString().equals("java.lang.Class") && targetValue instanceof Local) {
                        ...
                        else if (field.getType().toString().equals("java.lang.reflect.Method") && targetValue instanceof Local) {
                            List<Unit> methodDefs = findDefs(body, stmt, (Local) targetValue);
                            for (Unit methodDef: methodDefs) {
                                if (methodDef instanceof AssignStmt && ((AssignStmt) methodDef).getRightOp() instanceof InvokeExpr) {
                                    SootMethod methodDefMethod = ((InvokeExpr) ((AssignStmt) methodDef).getRightOp()).getMethod();
                                    if (methodDefMethod.getSignature().equals("<java.lang.Class: java.lang.reflect.Method getMethod(java.lang.String,java.lang.Class[])>")) {
                                        Value methodValue = ((InvokeExpr) ((AssignStmt) methodDef).getRightOp()).getArg(0);
                                        List<String> reflectedMethodNames = retrieveStringValue(body, (Stmt) methodDef, methodValue, new ArrayList<>(), false, detail);
                                        Value classValue = ((InvokeExpr) ((AssignStmt) methodDef).getRightOp()).getUseBoxes().get(0).getValue();
                                        List<Unit> classDefs = findDefs(body, (Stmt) methodDef, (Local) classValue);
                                        for (Unit classDef: classDefs) {
                                            if (classDef instanceof AssignStmt && ((AssignStmt) classDef).getRightOp() instanceof InvokeExpr) {
                                                ...
                                                if (classDefMethod.getSignature().equals("<java.lang.Class: java.lang.Class forName(java.lang.String)>")) {
                                                    Value firstArg = ((InvokeExpr) ((AssignStmt) classDef).getRightOp()).getArg(0);
                                                    List<String> reflectedClassNames = retrieveStringValue(body, (Stmt) classDef, firstArg, new ArrayList<>(), false, detail);
                                                    if (!reflectedClassNames.isEmpty() && AppConfig.acceptedClasses.stream().anyMatch(reflectedClassNames::contains)) {
                                                        AppConfig.fileResult.addMethod(targetMethod);
                                                        return true;
                                                    }
...
//        String md5 = Utils.calculateMD5("retrieveIteratorValue" + body.getMethod().getSignature() + stmt.toString() + iteratorArg.toString());
//        if (visited.contains(md5)) {
//            return result;
//        }
//        else {
